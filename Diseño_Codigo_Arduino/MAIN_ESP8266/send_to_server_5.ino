...
URL_withPacket += String(sensor_value);//valor del sensor
...
url += "&un=5";
url += "&b6=";
url +=  String(sent_bool_1);//valor del sensor
url += "&b7=";
url +=  String(sent_bool_2);//valor del sensor
url += "&b8=";
url +=  String(sent_bool_3);//valor del sensor
...
URL_withPacket += "Connection: close\r\n\r\n";  
 ...
 /*
  ...
  //we still have more data to get out of this stream, now we want d1
   if(read_until_ESP(keyword_b6,sizeof(keyword_b6),5000,0)){//lo mismo que antes - primer d1
   if(read_until_ESP(keyword_doublehash,sizeof(keyword_doublehash),5000,1)){//ahora ## y modo=1
   for(int i=1; i<=(scratch_data_from_ESP[0]-sizeof(keyword_doublehash)+1); i++)
   d6_from_ESP[i] = scratch_data_from_ESP[i];
   d6_from_ESP[0] = (scratch_data_from_ESP[0]-sizeof(keyword_doublehash)+1);

   if(read_until_ESP(keyword_b7,sizeof(keyword_b7),5000,0)){//lo mismo que antes - primer d1
   if(read_until_ESP(keyword_doublehash,sizeof(keyword_doublehash),5000,1)){//ahora ## y modo=1
   for(int i=1; i<=(scratch_data_from_ESP[0]-sizeof(keyword_doublehash)+1); i++)
   d7_from_ESP[i] = scratch_data_from_ESP[i];
   d7_from_ESP[0] = (scratch_data_from_ESP[0]-sizeof(keyword_doublehash)+1);

   if(read_until_ESP(keyword_b8,sizeof(keyword_b8),5000,0)){//lo mismo que antes - primer d1
   if(read_until_ESP(keyword_doublehash,sizeof(keyword_doublehash),5000,1)){//ahora ## y modo=1
   for(int i=1; i<=(scratch_data_from_ESP[0]-sizeof(keyword_doublehash)+1); i++)
   ...
   Serial.print("SENT_BOOL_1 = ");//imprime los datos del LED y conviértelos en números enteros
   ...
   Serial.print("SENT_BOOL_2 = ");//imprime los datos del LED y conviértelos en números enteros
   ...
   Serial.print("SENT_BOOL_3 = ");//imprime los datos del LED y conviértelos en números enteros
